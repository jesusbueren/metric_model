subroutine save_results(beta_h,beta_d,gamma,LE,fraction_t,it)
    ...
    real(dp),dimension(clusters*covariates*types*L_gender*L_educ,1)::c_tr
    real(dp),dimension(clusters*covariates*types*L_gender*L_educ,1)::c_tr_d
    real(dp),dimension(covariates_habits*habits*types,1)::c_habits
    real(DP),dimension(types*L_gender*L_educ*(clusters+1),1)::LE_v
    real(DP),dimension(generations*types*L_gender*L_educ,1)::fraction_t_v
    
    c_tr=reshape(beta_h,(/covariates*types*clusters*L_gender*L_educ,1/))
    c_tr_d=reshape(beta_d,(/covariates*types*clusters*L_gender*L_educ,1/))
    c_habits=reshape(gamma,(/covariates_habits*habits*types,1/))
    LE_v=reshape(LE,(/types*L_gender*L_educ*(clusters+1),1/))
    fraction_t_v=reshape(fraction_t,(/generations*types*L_gender*L_educ,1/))
