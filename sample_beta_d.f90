...
subroutine sample_beta_d(beta_d,type_i,sample_k)
    ...
    real(DP),dimension(indv*g_max,clusters,types,L_gender,L_educ,covariates)::big_X_d
    real(DP),dimension(indv*g_max,clusters,types,L_gender,L_educ)::big_Y_d
    integer,dimension(clusters,types,L_gender,L_educ)::counter_big_X_d
    interface
    ...
    do i_l=1,indv_HRS;do g_l=first_age(i_l),last_age(i_l)-1
            ...
            if (sample_k(i_l,g_l)>=1 .and. sample_k(i_l,g_l+1)>=1 .and. race(i_l)==1) then
                counter_big_X_d(sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l))=counter_big_X_d(sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l))+1
                big_X_d(counter_big_X_d(sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l)),sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l),:)=x(:,1)
                if (sample_k(i_l,g_l+1)==clusters+1 ) then
                    call TRUNCATED_NORMAL_A_SAMPLE(sum(x(:,1)*beta_d(:,type_i(i_l,1),sample_k(i_l,g_l),gender(i_l),educ(i_l))),1.0_dp,0.0_dp,d_star)
                elseif (sample_k(i_l,g_l+1)<clusters+1  ) then
                    call TRUNCATED_NORMAL_B_SAMPLE(sum(x(:,1)*beta_d(:,type_i(i_l,1),sample_k(i_l,g_l),gender(i_l),educ(i_l))),1.0_dp,0.0_dp,d_star)
                end if
                if (d_star>100 .or. d_star<-100) then
                    print*,d_star
                    print*,x(:,1)
                    print*,beta_d(:,type_i(i_l,1),sample_k(i_l,g_l),gender(i_l),educ(i_l))
                    read*,pause_k
                end if
                big_Y_d(counter_big_X_d(sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l)),sample_k(i_l,g_l),type_i(i_l,1),gender(i_l),educ(i_l))=d_star
            end if
