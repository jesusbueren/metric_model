    ...
    do i_l=1,indv;
        do g_l=first_age(i_l),last_age(i_l);do h_l=1,habits_nomed
            if (sample_k(i_l,g_l)/=-1) then
                ...
                if ((data_habits(i_l,habits_vec(h_l),g_l)==1 .or. data_habits(i_l,habits_vec(h_l),g_l)==0) .and. race(i_l)==1) then
                    counter_big_X(type_i(i_l,1),h_l)=counter_big_X(type_i(i_l,1),h_l)+1
                    big_X(counter_big_X(type_i(i_l,1),h_l),type_i(i_l,1),h_l,:)=x(:,1)
                end if
                if (data_habits(i_l,habits_vec(h_l),g_l)==1 .and. race(i_l)==1) then
                ...
                elseif (data_habits(i_l,habits_vec(h_l),g_l)==0 .and. race(i_l)==1) then
    ...
    do i_l=1,indv_HRS;
        do g_l=first_age(i_l),last_age(i_l);do h_l=1,habits_med
            if (sample_k(i_l,g_l)/=-1 .and. data_ins_hrs(i_l,g_l)/=-1) then
                ...
                if ((data_habits(i_l,habits_med_vec(h_l),g_l)==1 .or. data_habits(i_l,habits_med_vec(h_l),g_l)==0) .and. race(i_l)==1) then
                    counter_big_X(type_i(i_l,1),h_l)=counter_big_X(type_i(i_l,1),h_l)+1
                    big_X(counter_big_X(type_i(i_l,1),h_l),type_i(i_l,1),h_l,:)=x(:,1)
                end if
                if (data_habits(i_l,habits_med_vec(h_l),g_l)==1 .and. race(i_l)==1) then
                    call TRUNCATED_NORMAL_A_SAMPLE(sum(x(:,1)*gamma_med(:,h_l,type_i(i_l,1))),1.0_dp,0.0_dp,y_star(i_l,h_l,g_l))
                    big_Y(counter_big_X(type_i(i_l,1),h_l),type_i(i_l,1),h_l)=y_star(i_l,h_l,g_l)
                elseif (data_habits(i_l,habits_med_vec(h_l),g_l)==0 .and. race(i_l)==1) then
