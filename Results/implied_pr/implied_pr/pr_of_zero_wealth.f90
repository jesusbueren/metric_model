subroutine pr_of_zero_wealth(type_pr,sample_k,pr_zero)
    ...
    real(DP),dimension(generations,types,L_educ,clusters,cohorts,iterations)::pr_zero_it
    integer::it
    ...
    do it=1,iterations    
        ...
        call sample_probit_p(y,sample_k,beta_w)
        
        !Compute posterior for implied pr of zero
        call zero_pr_hat(beta_w,pr_zero_it(:,:,:,:,:,it)) !pr_zero(:,1,3,2)
    
    end do
    
    pr_zero=sum(pr_zero_it,6)/dble(iterations)
