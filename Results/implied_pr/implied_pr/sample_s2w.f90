    ...
    do e_l=1,L_educ
        ...
        do i_l=indv_HRS+1,indv
            do t_l=first_age(i_l),last_age(i_l)
                if (u_draw(i_l,t_l)/=-1.0d0 .and. gender(i_l)==1 .and. initial_age+(t_l-1)*2<60    .and. educ(i_l)==e_l .and. data_income(i_l,t_l)>520.0d0*7.25d0) then   ! .and. 
                    ind=ind+1
                    ...
                    x(1:covariates_mix_mean,1)=(/1.0_dp,dble(age),dble(age)**2.0d0,dble(age)**3.0d0,dble(data_shlt(i_l,t_l)-1),dble(age)*dble(data_shlt(i_l,t_l)-1)/)
        ...
        shape=v/2
        scale=1/(v*s2/2)
        s2_w(e_l,:)=1/(r8_gamma_01_sample(shape)*scale)
    end do
