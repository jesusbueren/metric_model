subroutine pr_of_zero_income_dynamic()
    ...
    real(DP),dimension(generations,clusters,L_educ,2)::pr_zero
    real(DP),dimension(covariates_mix_d,clusters,L_educ)::beta_i
    integer,parameter::iterations=500
    real(DP),dimension(generations,clusters,L_educ,2,iterations)::pr_zero_it
    integer::it,e_l,h_l,g_l,lf_l
    ...
    open(unit=10,file=path//"metric_model\Results\labor_force_participation_dynamic.txt")
         do lf_l=1,2; do e_l=1,L_educ; do h_l=1,clusters;do g_l=1,generations
            write(10,'(I3,I3,I3,F10.5)') h_l,e_l,g_l,1.0d0-pr_zero(g_l,h_l,e_l,lf_l)
