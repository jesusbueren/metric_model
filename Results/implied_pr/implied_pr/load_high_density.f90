subroutine load_high_density(c_mean,gamma_mean)
    ...
    implicit none
    double precision,dimension(covariates,types,clusters,clusters+1),intent(out)::c_mean
    real(DP),dimension(covariates_habits,habits,types),intent(out)::gamma_mean
    integer,parameter::burn=200
    integer,parameter::iterations=2317
    double precision,dimension(clusters**2*types*covariates,iterations)::c_vec
    ...
    c_mean=0.0d0
    c_mean(:,:,:,1:clusters)=reshape(sum(c_vec(:,burn:iterations),2)/dble(iterations-burn+1),(/covariates,types,clusters,clusters/))
