subroutine quantile_wealth_hat(h_l,y_l,e_l,g_l,c_l,beta_mean,beta_var,quantile,q_wealth,mean_w,variance_w)
    use global_var; use nrtype; use mixtures_vars
    implicit none
    integer,intent(in)::y_l,e_l,g_l,h_l,c_l
...
subroutine quantile_income_hat(y_l,e_l,h_l,g_l,beta_mean,beta_var,quantile,q_income,mean)
    use global_var; use nrtype; use mixtures_vars_income
    implicit none
    integer,intent(in)::y_l,e_l,g_l,h_l
    ...
    real(DP)::x2
        ...
        cohort_d=0.0d0
        cohort_d(1)=1.0d0
        x(1:covariates_mix_mean,1)=(/1.0_dp,dble(age),dble(age)**2.0d0,dble(age)**3.0d0,dble(h_l-1),dble(h_l-1)*dble(age)/)
        x2=2.0d0*quantile-1.0d0
        q_income=exp(sum(x(:,1)*beta_mean(:,e_l))+sqrt(beta_var)*sqrt(2.0d0)* &
            sqrt(pi_d)*(0.5d0*x2+1.0d0/24.0d0*pi_d*x2**3.0d0+7.0d0/960.0d0*pi_d**2.0d0*x2**5.0d0+127.0d0/80640.0d0*pi_d**3.0d0*x2**7.0d0))
        
        mean=sum(x(:,1)*beta_mean(:,e_l))
