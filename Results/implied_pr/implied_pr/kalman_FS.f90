subroutine kalman_FS(rho,s2_nu,s2_w,beta_mean,y,u_draw)
    ...
    do i_l=indv_HRS+1,indv
        ...
        do t_l=first_age(i_l),last_age(i_l)
            ...
            y_p(t_l)=sum(x(1:covariates_mix_mean,1)*beta_mean(:,y(i_l,1),educ(i_l)))+xi_p(t_l)+c4_normal_01( )*sqrt(s2_w)
            ...
            if (data_income(i_l,t_l)>520.0d0*7.25d0 .and. gender(i_l)==1 .and. initial_age+(t_l-1)*2<64) then
                ...
                xi_t1_t1(t_l)=xi_t1_t0(t_l)+K*(log(data_income(i_l,t_l))-sum(x(1:covariates_mix_mean,1)*beta_mean(:,y(i_l,1),educ(i_l)))-xi_t1_t0(t_l)) 
            else
                K=0.0d0
                xi_t1_t1(t_l)=xi_t1_t0(t_l)
            end if
            xi_t1_t1_p(t_l)=xi_t1_t0_p(t_l)+K*(y_p(t_l)-sum(x(1:covariates_mix_mean,1)*beta_mean(:,y(i_l,1),educ(i_l)))-xi_t1_t0_p(t_l))
