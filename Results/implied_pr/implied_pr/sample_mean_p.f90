...
subroutine sample_mean_p_income(y,beta_var,u_draw,beta_mean)
    ...
    real(DP),dimension(covariates_mix_mean,types,L_educ),intent(out)::beta_mean
    integer::i_l,g_l,e_l,y_l,c_l
    real(DP)::age
    real(DP),dimension(covariates_mix_mean,1)::x,z
    integer,dimension(types,L_educ)::counter_big_X
    real(DP),dimension(indv*10,types,L_educ,covariates_mix_mean)::big_X
    real(DP),dimension(indv*10,types,L_educ)::big_Y
    ...
    do i_l=indv_HRS+1,indv;do g_l=first_age(i_l),last_age(i_l)
        age=initial_age+(g_l-1)*2-70
        x(1:covariates_mix_mean,1)=(/1.0_dp,dble(age),dble(age)**2.0d0,dble(age)**3.0d0,dble(data_shlt(i_l,g_l)-1)/)     
        if (data_income(i_l,g_l)>520.0d0*7.25d0 .and. gender(i_l)==1 .and. initial_age+(g_l-1)*2<64) then
            counter_big_X(y(i_l,1),educ(i_l))=counter_big_X(y(i_l,1),educ(i_l))+1
            big_X(counter_big_X(y(i_l,1),educ(i_l)),y(i_l,1),educ(i_l),:)=x(:,1)
            big_Y(counter_big_X(y(i_l,1),educ(i_l)),y(i_l,1),educ(i_l))=log(data_income(i_l,g_l))-u_draw(i_l,g_l)
        end if
    end do; end do
