...
plot(squeeze(c_delta(c_l,1,e_l,burn:end)))
end
end
...
for e_l=1:3
subplot(1,3,e_l)
plot(squeeze(fraction_t(:,1,e_l,:,c_l,end)))
ylim([0 1])
end
...
plot(Av_LE_c(1,:))
subplot(1,3,2)
plot(squeeze(fraction_t(12,ge_l,1,types,c_l,burn:iterations)))
hold on
plot(squeeze(fraction_t(12,ge_l,3,types,c_l,burn:iterations)))
subplot(1,3,3)
...
for e_l=1:3
    ...
    for p_l=1:types 
        ...
        h(p_l)=errorbar(26:4:92,mean(squeeze(H(h_l,h_l2,1:2:34,p_l,ge_l,e_l,burn:end)),2),2.*std(squeeze(H(h_l,h_l2,1:2:34,p_l,ge_l,e_l,burn:end))'),...
            ...
            h(p_l)=errorbar(26:4:92,mean(squeeze(H(h_l,h_l2,1:2:34,p_l,ge_l,e_l,burn:end)./(1.0-H(h_l,3,1:2:34,p_l,ge_l,e_l,burn:end))),2),2.*std(squeeze(H(h_l,h_l2,1:2:34,p_l,ge_l,e_l,burn:end)./(1.0-H(h_l,3,1:2:34,p_l,ge_l,e_l,burn:end)))'),...
            marker{p_l},'MarkerSize',6,'MarkerFaceColor',colors{p_l})
        end
...
%% Fraction of the gradient explain by groups across different models
...
for types=2:4
    ...
    fraction_explained(:,types)=(Av_LE_c(1,:)-Av_LE(1,:))./(Av_LE(3,:)-Av_LE(1,:));

    h=errorbar(types,median(fraction_explained(:,types)),1.95*std(fraction_explained(:,types)), ...
        marker{2},'MarkerSize',6,'MarkerFaceColor',colors{2});
    h.Color = colors{2};
    hold on
