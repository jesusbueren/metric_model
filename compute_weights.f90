subroutine compute_weights(delta,H,share_h,weights,joint_yh) 
    ...
    implicit none
    real(DP),dimension(clusters,L_gender,L_educ,types),intent(in)::delta
    ...
    do e_l=1,L_educ;do ge_l=1,L_gender
        ...
        do h_l=1,clusters
            do y_l=1,types
                weights(g_l,h_l,ge_l,e_l,y_l)=delta(h_l,ge_l,e_l,y_l)
                joint_yh(g_l,h_l,ge_l,e_l,y_l)=weights(g_l,h_l,ge_l,e_l,y_l)*share_h(h_l,ge_l,e_l)
            end do 
        end do
