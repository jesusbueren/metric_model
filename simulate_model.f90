subroutine simulate_model()
    ...
    real(DP),dimension(generations,clusters,L_gender,L_educ,types,cohorts)::weights,joint_yh !p(y|h) p(y,h)
    ...
    character(len=4)::table_name
        ...
        beta_h=sum(beta_h_all,5)/dble(iterations)
        beta_d=sum(beta_d_all,5)/dble(iterations)
        ! Given health and survival parameters compute health transitions
        joint_yh=1.0d0/dble(clusters*types) 
        call transitions(beta_h,beta_d,H,LE,joint_yh) !LE(:,1,1,3)
        
        !Compute the joint pr(y,h|a,e,s,c)
        delta=sum(delta_all,5)/dble(iterations)
        ...
        gamma=sum(gamma_all,4)/dble(iterations)
...
open(unit=10,file=path_s2//'table_1_'//table_name//'.txt')
    ...
    write(10,'(A100)'),'  & \textsc{hsd} & \textsc{hsg} & \textsc{cg} &  50-60 & 70-80 &  50-60 & 70-80 \\'
    ...
    do h_l2=1,habits
        ...
        '\textbf{',str_habit(h_l),'}&',pr_h_e(h_l,1),'&',pr_h_e(h_l,2),'&',pr_h_e(h_l,3),'&' &
...
do h_l2=1,habits
    h_l2o=order(h_l2)
    if (h_l2<habits) then
        write(10,'(A10,A11,A1)'),'& \textbf{',str_habit(h_l2o),'} '  
    else
        write(10,'(A10,A11,A4)'),'& \textbf{',str_habit(h_l2o),'} \\'
    end if
end do
write(10,'(A20)'),'\midrule' 
do h_l1=1,habits
    h_l1o=order(h_l1)
    write(10,'(A8,A11,A1)'),'\textbf{',str_habit(h_l1o),'} '  
    do h_l2=1,habits
        h_l2o=order(h_l2) 
        if (h_l1>h_l2) then
            if (h_l2<habits) then
